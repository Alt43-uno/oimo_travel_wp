	...
	if ( isset( $post ) && ! is_null( $post ) ) {
		...
		if ( isset( $post_meta_infos[0] ) ) {
			foreach ( $post_meta_infos as $meta_info ) {
				...
				$sanitized_meta_value = wp_strip_all_tags( $meta_info->meta_value );

				$wpdb->query( $prepared_statement, array( $meta_key, $sanitized_meta_value ) );
			}
		}
	} else {
		return null;
...
function wptravelengine_svg_by_fa_icon( $icon_name, $echo = true, $class_names = array() ) {
	...
	$path = isset( $data[ $icon_name ] ) ? $data[ $icon_name ] : '';
...
function wptravelengine_payment_status( $status = null ) {
	...
	return isset( $options[ $status ] ) ? $options[ $status ] : __( 'N/A', 'wp-travel-engine' );
}
	...
	foreach ( $_menu as $index => $item ) {
		if ( ( $item[2] === 'edit.php?post_type=booking' ) || $add_bottom_separator ) {
			if ( substr( $item[2], 0, 9 ) !== 'separator' ) {
				$menu[ ( ( $previous_index + $index ) / 2 ) . '' ] = array( '', 'read', "separator{$index}", '', 'wp-menu-separator' );
			}
		}
