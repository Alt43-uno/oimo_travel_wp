...
class Wp_Travel_Engine_Admin {
	...
	public function add_booking_export_button() {
		...
		if ( isset( $_GET['post_type'] ) && 'booking' === $_GET['post_type'] && 'booking' == $post_type ) {
			...
			<form id="wpte-booking-export-form" class="wpte-export-form" method="post">
				...
				<input type="text" name="wte_booking_range" data-fpconfig='{"mode":"range","showMonths":"2"}' name="wte-flatpickr__date" value="<?php echo esc_attr( isset( $_POST['wte_booking_range'] ) ? $_POST['wte_booking_range'] : '' ); ?>" class="wte-flatpickr">
				<input type="submit" name="booking_export_submit" class="button button-primary" value="<?php esc_html_e( 'Export Bookings', 'wp-travel-engine' ); ?>">
			</form>
			<?php
		}
		?>
			...
			switch ( $column ) {
				...
				case 'created':
					...
					if ( is_array( $my_bookings ) && ! empty( $my_bookings ) ) {
						...
						if ( isset( $query[0] ) ) {
							echo wp_date( 'Y-m-d', strtotime( $query[0]->post_date ) );
							break;
						}
					}
	...
	*/
		...
		<div style="margin-top:40px;" class="wpte-form-block">
			...
			if ( class_exists( 'Wte_Itinerary_Downloader' ) ) {
				...
				<div class="wpte-shortcode">
					<span class="wpte-tooltip"><?php esc_html_e( 'To display Itinerary Downloader in current trip page use the following', 'wp-travel-engine' ); ?> <b><?php esc_html_e( 'Shortcode.', 'wp-travel-engine' ); ?></b></span>
					<div class="wpte-field wpte-field-gray wpte-floated">
						<input id="wpte-iten-down-code" readonly type="text" value="<?php echo esc_attr( $page_shortcode ); ?>">
	...
	public function wte_add_dashboard_widget() {
		wp_add_dashboard_widget( 'wpte_dashboard_widget', 'WP Travel Engine Booking Summary', array( $this, 'wpte_dashboard_widget_function' ) );
	}
	...
	public function wpte_dashboard_widget_function() {
		...
		$help_url      = 'https://wptravelengine.com/support-ticket/?utm_source=dashboard&utm_medium=customer_site&utm_id=dashboard';
		$gopro_url     = 'https://wptravelengine.com/pricing/?utm_source=dashboard&utm_medium=customer_site&utm_id=dashboard';
		if ( isset( $response_data ) && count( $response_data ) > 0 ) {
			...
			<span class="wpte-dashboard-widget_heading"><?php esc_html_e( 'New Customers', 'wp-travel-engine' ); ?></span>
			<div class="wpte-dashboard-widget_customers-status">
				<?php if ( isset( $customer_range['mtd'] ) ) : ?>
					<div class="wpte-dashboard-widget_status-item">
						...
						<strong class="value"> <?php echo esc_html( $customer_range['mtd'][0] ); ?> <span class="status-<?php echo esc_attr( $customer_range['mtd'][2] ); ?>">(<?php echo esc_html( $customer_range['mtd'][1] ); ?>)</span> </strong>
					</div>
				<?php endif; ?>
				...
				<?php if ( isset( $customer_range['yoy'] ) ) : ?>
					<div class="wpte-dashboard-widget_status-item">
						...
						<strong class="value"> <?php echo esc_html( $customer_range['yoy'][0] ); ?> <span class="status-<?php echo esc_attr( $customer_range['yoy'][2] ); ?>">(<?php echo esc_html( $customer_range['yoy'][1] ); ?>)</span> </strong>
					</div>
				<?php endif; ?>
				...
				<?php if ( isset( $customer_range['yom'] ) ) : ?>
					<div class="wpte-dashboard-widget_status-item">
						...
						<strong class="value"> <?php echo esc_html( $customer_range['yom'][0] ); ?> <span class="status-<?php echo esc_attr( $customer_range['yom'][2] ); ?>">(<?php echo esc_html( $customer_range['yom'][1] ); ?>)</span> </strong>
					</div>
				<?php endif; ?>
			</div>

			<span class="wpte-dashboard-widget_heading"><?php esc_html_e( 'Recent Bookings', 'wp-travel-engine' ); ?></span>
			<ul>
					...
					<?php foreach ( $customer_data as $customers ) : ?>
						<li>
							<?php if ( isset( $customers['url'], $customers['title'], $customers['booked_trip'], $customers['date'] ) ) : ?>
								<a href="<?php echo esc_url( $customers['url'] ); ?>" target="_self"><?php echo esc_attr( $customers['title'] ) . ' booked ' . esc_attr( $customers['booked_trip'] ); ?></a>
			...
			<a href="<?php echo esc_url( $bookings_url ); ?>" class="button button-primary wpte-dashboard-widget_button"><?php esc_html_e( 'View All Bookings', 'wp-travel-engine' ); ?></a>
			<div class="wpte-dashboard-widget_footer-links">
				<a href="<?php echo esc_url( $tutorials_url ); ?>" target="_blank"><?php esc_html_e( 'Tutorials', 'wp-travel-engine' ); ?></a>
				<a href="<?php echo esc_url( $help_url ); ?>" target="_blank"><?php esc_html_e( 'Help', 'wp-travel-engine' ); ?></a>
				<a href="<?php echo esc_url( $gopro_url ); ?>" target="_blank" class="text-green"><?php esc_html_e( 'Go Pro', 'wp-travel-engine' ); ?></a>
			</div>
			<?php
		}
	}
