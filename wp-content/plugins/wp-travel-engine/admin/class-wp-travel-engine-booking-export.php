...
class WP_Travel_Engine_Booking_Export {
	...
	public function export_query( $start_date, $end_date ) {
		...
		$sql = " SELECT p.ID AS BookingID, (SELECT pm1.meta_value FROM $wpdb->postmeta pm1 WHERE pm1.post_id = p.ID AND pm1.meta_key = %s LIMIT 1) AS BookingStatus, (SELECT pm2.meta_value FROM $wpdb->postmeta pm2 WHERE pm2.post_id = p.ID AND pm2.meta_key = %s LIMIT 1) AS placeorder, (SELECT pm3.meta_value FROM $wpdb->postmeta pm3 WHERE pm3.post_id = p.ID AND pm3.meta_key = %s LIMIT 1) AS billinginfo, SUM(pm.meta_value) AS TotalCost, SUM(CASE WHEN pm.meta_key = %s THEN pm.meta_value ELSE 0 END) AS TotalPaid, (SELECT pm4.meta_value FROM $wpdb->postmeta pm4 WHERE pm4.post_id = p.ID AND pm4.meta_key = %s LIMIT 1) AS PaymentGateway, (SELECT pm5.meta_value FROM $wpdb->postmeta pm5 WHERE pm5.post_id = p.ID AND pm5.meta_key = %s LIMIT 1) AS wc_id, DATE_FORMAT(p.post_date, '%Y-%m-%d') AS BookingDate FROM $wpdb->postmeta pm INNER JOIN $wpdb->posts p ON pm.post_id = p.ID WHERE pm.meta_key IN (%s) AND p.post_type = %s AND p.post_status IN (%s) ";

		$params = array_merge(
			array( $meta_keys[0], $meta_keys[1], $meta_keys[2], $meta_keys[3], $meta_keys[4], $meta_keys[5], $meta_keys[6], 'booking', 'publish', 'draft' )
		);
		...
		$queries_data = $wpdb->get_results( $wpdb->prepare( $sql, $params ) );
