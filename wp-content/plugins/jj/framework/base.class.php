...
class RsAddOnTransitionpackBase {
	...
	protected function loadClasses() {
		...
		new SrTransitionPackFront(static::$_PluginTitle);
	...
	public function enqueue_admin_scripts($hook) {
		...
		if($hook === 'toplevel_page_revslider') {
			...
			$_jsPathMin = file_exists(RS_TRANSITIONPACK_PLUGIN_PATH . 'sr6/assets/js/revolution.addon.' . static::$_PluginTitle . '.js') ? '' : '.min';	
	...
	public static function add_transitions($transitions){
		...
		// FLUID EFFECTS
		$transitions['tpack']['tpcolore'] = array(
			'tp_webgl_color' => array('eng' => 'transitionPack','e' => 'tpbasic',
					'title' => 'Color Melt','speed' => 1500,'d' => 20,'addOns' => array(
						'tpack' => array('ef' => 'colormelt','dplm' => '8','rad' => 90,'w' => 35)))
					);
...
/*OWN*/		'colormelt' => "`uniform float progress;uniform sampler2D src1;uniform sampler2D src2;uniform vec4 resolution;uniform float shake;uniform float amo;uniform float sd;uniform float Cs;uniform float rMin;uniform float rMax;uniform float gMin;uniform float gMax;uniform float bMin;uniform float bMax;varying vec2 vUv;const int passes = 6;float pi = 3.141592653;vec4 getFromColor(vec2 uv) {return TEXTURE2D(src1, uv);}vec4 getToColor(vec2 uv) {return TEXTURE2D(src2, uv);}float rnd(float n, float seed){return sin(cos(n * 5823. * seed) * 3145.);}float circle(vec2 pos,float size, vec2 p){return smoothstep(0.0,1.0,(size-length(p-pos)));}vec3 blur(sampler2D tex,vec2 p, float size, float amp){        vec3 color = vec3(0.0);float am = 1.0/pow(size*2.,2.);for(float i = -size;i <size;i++){for(float j = -size;j <size;j++){vec2 newp = p + vec2(i,j) * 0.001 * amp;color += vec3(texture(tex,newp).xyz);}}    color *= am;    p += rnd(p.x,p.y)*0.002 * amp;    color += vec3(texture(tex,p).xyz);    return color * 0.5;    }void main(){    vec2 fragCoord = vUv;float iTime = progress*4.;        vec2 iResolution = vec2(resolution.z, resolution.w); vec2 p = (fragCoord*2.0-iResolution.xy)/iResolution.y;vec2 q;vec3 col;    float speed = 0.5;float strength = shake * (smoothstep(0.,1.0,4.-iTime));    vec2 ColR = vec2(rMin,rMax);vec2 ColG = vec2(gMin,gMax);vec2 ColB = vec2(bMin,bMax);   bool DirNotRandom = false;vec2 NewDir = vec2(0.0,-1.0);float EFCS = 0.7;vec2 colAmpX = vec2(1.); float t = speed * iTime;q.x = rnd(5.,2. + floor(t) * 0.01) * 0.1;q.y = rnd(5.,1. + floor(t) * 0.01) * 0.1;vec2 dir = normalize(q);p += dir * sin(fract(t) * pi * 2.) * pow(sin(fract(t)*pi+pi),1.) * 0.1 * strength;p *= 1.0 - sin(iTime*pi*0.25)*0.1;float effSt = sin(smoothstep(0.,1.,iTime*0.25 * speed * 2.)*pi);col += blur(src2,(p * iResolution.y + iResolution.xy)*0.5/iResolution.xy,10.,3. * effSt) * smoothstep(0.0,1.0,iTime*0.25);col += blur(src1,(p * iResolution.y + iResolution.xy)*0.5/iResolution.xy,10.,3. * effSt) * smoothstep(1.0,0.0,iTime*0.25);vec3 CirclCol = vec3(0.); for(float i = 0.;i<amo;i++){vec2 cq = vec2(rnd(i*5.32*sd,i*1.42*sd),rnd(i*3.32,i*0.42*sd));vec2 Cdir = vec2(rnd(i*0.83*sd,i*0.89*sd),rnd(i*0.51*sd,i*0.82*sd));if(DirNotRandom){cq += NewDir * -1.0;Cdir = NewDir;}float C1 = rnd(cq.x*0.841,cq.y*8.459*sd) * (ColR.y-ColR.x) + (ColR.x+ColR.y)* 0.5;float C2 = rnd(cq.x*4.615,cq.y*2.195*sd) * (ColG.y-ColG.x) + (ColG.x+ColG.y)* 0.5;float C3 = rnd(cq.x*1.517,cq.y*3.315*sd) * (ColB.y-ColB.x) + (ColB.x+ColB.y)* 0.5;float Csize = rnd(cq.x*1.517,cq.y*3.315) * .1 + Cs;vec3 Rcol = vec3(C1,C2,C3) * .5;float s = 20.-iTime*10.;float StC = smoothstep(s-3.0,s,i);cq += Cdir * pow(iTime*0.5,3.0) ;cq.x *= 1.2;CirclCol *=  1.-circle(cq,.5,p);CirclCol += 1.*Rcol * circle(cq,Csize,p) * sin(smoothstep(0.,1.,iTime*0.5 * speed + 0.25)*pi) * StC;CirclCol *= 0.5 + smoothstep(0.,Csize*EFCS,length(p-cq)) * 0.5;}col += CirclCol;gl_FragColor = vec4(col,1.0);}`"
		);
		return $shaders;
	}
	...
	public static function get_var() {
		...
		return array(
			...
			'bricks' => array(
				...
				'tpcolore' => __('Color Effects', $_textdomain),
