<?php
class WTE_Fixed_Starting_Dates_Meta_Tabs{
	...
	private static function tabs(){
		...
		if( !empty( $args ) ){
			...
			foreach ($args['tabs'] as $key => $tab) {
				...
				$output .= sprintf( '<a href="javascript:void(0);" class="nav-tab tm-tab-trigger %s" data-configuration="%s">%s</a>', $class, $tab['id'], $tab['title'] );
			}
			$output .= '</h2>';
	...
	private static function content(){
		...
		if( !empty( $args ) ){
			...
			foreach ($args['tabs'] as $key => $tab) {
				...
				$output .= sprintf( '<div class="tm-%s-configurations tm-configurations" style="%s">', $tab['id'], $style );
				$output .= $tab['content'];
				$output .= '</div>';
			}
			$output .= '</div>';
