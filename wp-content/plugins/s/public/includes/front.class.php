...
class SrSnowFront extends RevSliderFunctions {
	...
	public function get_script_list(){
		$min = file_exists(RS_SNOW_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';

		return array('snow' => RS_SNOW_PLUGIN_URL.'public/js/' . $this->slug . $min . '.js');
	}
	...
	public function is_in_use($slider){
		...
		$return = (strpos($json, 'revslider-'.$this->slug.'-addon') !== false || (strpos($json, 'var snowsettings') !== false)) ? true : false;
		unset($json);
	...
	public function enqueue_scripts(){
		...
		$this->script_enqueued = true;
		$min = file_exists(RS_SNOW_PLUGIN_PATH . 'public/js/' . $this->slug . '.js') ? '' : '.min';
		wp_enqueue_script('revslider-'.$this->slug.'-addon', RS_SNOW_PLUGIN_URL . "public/js/" . $this->slug . $min . ".js", '', RS_REVISION, array('strategy' => 'async'));
	}
	...
	public function add_addon_files($html, $export){
		...
		foreach($list ?? [] as $handle => $script){
			$script_path = str_replace(RS_SNOW_PLUGIN_URL, RS_SNOW_PLUGIN_PATH, $script);
			if(!$export->usepcl){
				$export->zip->addFile($script_path,  $export_path_js . $handle . '.js');
			}else{
				$export->pclzip->add($script_path, PCLZIP_OPT_REMOVE_PATH, RS_SNOW_PLUGIN_PATH.'public/js/', PCLZIP_OPT_ADD_PATH, $export_path_js);
			}
